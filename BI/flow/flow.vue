<style lang="less">

  .flow-editor-html {
    position: relative;
    width: 100%;
    // height: 100%;
    height: 687px;

    .tool-bar {
      position: absolute;
      padding: 5px 20px;
      width: 100%;
      border: 1px solid #e9e9e9;
      height: 42px;
      display: flex;
      justify-content: space-between;
      align-items: center;
      background: #ffffff;
      box-shadow: 0px 8px 12px 0px rgba(0, 52, 107, 0.04);
      .toolbar {
        height: 100%;
        width: 80%;

        >a{
          position: relative;
          display: inline-block;
          width: 33px;
          height: 33px;
          padding: 0px 15px;
          color: currentColor;
        }

        span.cut{       
          opacity: 0.5;
          cursor: auto;
        }

        .command,.custom {
          width: 100%;
          height: 100%;
          line-height: 33px;
          display: block;
          position: absolute;
          text-align: center;
          top: 0px;
          left: 0px;           
          // top: 9px;
          // position: absolute;
          // left: 12px;

        }

        .disable {
          color: rgba(0, 0, 0, 0.25);
        }
      }
      .data-opt {
        width: 20%;
        height: 100%;
        text-align: right;
      }
    }

    .page-container {
      padding-top: 41px;
      width: 100%;
      height: 100%;
      display: flex;
      background: #f7f9fb;
      justify-content: space-around;
      .title {
        display: flex;
        width: 100%;
        align-items: center;
        height: 34px;
        font-weight: 600;
        font-size: 16px;
        padding: 0 20px;
        background: #cfd4d9;
      }
      .left-pannel {
        width: 120px;
        height: 100%;
        padding: 20px;
        display: inline-flex;
        flex-direction: column;
        align-items: center;
        .itempannel-container {

          img {
            width: 82px;
            height: 82px;
            padding: 4px;
            border-radius: 2px;
            border: 1px solid rgba(0, 0, 0, 0);
            vertical-align: middle;
            cursor: pointer;
          }
        }
      }
      .center-pannel {
        background: #ffffff;
        display: inline-block;
        width: 80%;
        height: 100%;
        border: 1px solid #dedbe2;
        .flow {
          width: 100%;
          height: 100%;
          overflow: hidden;
        }
        .contextmenu {
          margin: 0px;
          width: 200px;
          background: white;
          box-shadow: 0px 1px 4px rgba(0, 0, 0, 0.25);
          color: #000;
          font-size: 12px;
          display: none;
          .command {
            height: 12px;
            padding: 8px;
            box-sizing: content-box;
          }
          .command:hover {
            cursor: pointer;
            background: #e6f7ff;
          }
          .disable {
            color: rgba(0, 0, 0, 0.25);
          }
        }
      }
      .right-pannel {
        width: 300px;
        height: 100%;
        display: inline-flex;
        flex-direction: column;
        .detail-pannel {
          width: 100%;
          height: 60%;
          background: #f7f9fb;
          border-left: 1px solid #e6e9ed;
          font-size: 13px;
          .pannel-title {
            height: 32px;
            border-top: 1px solid #dce3e8;
            border-bottom: 1px solid #dce3e8;
            background: #ebeef2;
            color: #000;
            line-height: 28px;
            padding-left: 12px;
          }
          .block-container {
            padding: 16px 8px;

            .ivu-row{
              padding-bottom: 10px;

              .ivu-col.label{
                text-align: right;
                padding: 7px 10px 0px 0px;
              }
            }

          }
        }
        .navigator {
          width: 100%;
          height: 30%;
          flex-grow: 1;
          .pannel-title {
            display: block;
            height: 32px;
            border-top: 1px solid #dce3e8;
            border-bottom: 1px solid #dce3e8;
            background: #ebeef2;
            color: #000;
            line-height: 28px;
            padding-left: 12px;
          }
          .mini-map {
            width: 100%;
            height: calc(100% - 34px);
          }
        }
        .zoom-slider {
          width: 100%;
          height: 5%;
          padding: 5px 10px;
          background: #ffffff;
        }
      }
    }
  }
</style>
<template>
  <div class="flow-editor-html">
    <div class="tool-bar">
      <div class="toolbar" ref="toolbar">
        <a href="javascript:;"><i data-command="undo" class="command iconfont icon-undo" title="撤销"></i></a>
        <a href="javascript:;"><i data-command="redo" class="command iconfont icon-redo disable" title="重做"></i></a>
        <span class="separator"></span>
        <a href="javascript:;"><i data-command="copy" class="command iconfont icon-copy-o" title="复制"></i></a>
        <a href="javascript:;"><i data-command="paste" class="command iconfont icon-paster-o disable" title="粘贴"></i></a>
        <a href="javascript:;"><i data-command="delete" class="command iconfont icon-delete-o" title="删除"></i></a>
        <a href="javascript:;"><i data-command="clear" class="command iconfont icon-trash-fill" title="清空画布"></i></a>
            <a href="javascript:;"><span class="cut custom iconfont icon-cut-off"></span></a>
        <!-- <span class="separator"></span> -->
        <a href="javascript:;"><i data-command="zoomIn" class="command iconfont icon-zoom-in-o" title="放大"></i></a>
        <a href="javascript:;"><i data-command="zoomOut" class="command iconfont icon-zoom-out-o" title="缩小"></i></a>
        <a href="javascript:;"><i data-command="autoZoom" class="command iconfont icon-fit" title="适应画布"></i></a>
        <a href="javascript:;"><i data-command="resetZoom" class="command iconfont icon-actual-size-o" title="实际尺寸"></i></a>
        <!-- <span class="separator"></span> -->
        <a href="javascript:;"><i data-command="toBack" class="command iconfont icon-to-back" title="层级后置"></i></a>
        <a href="javascript:;"><i data-command="toFront" class="command iconfont icon-to-front" title="层级前置"></i></a>
        <!-- <span class="separator"></span> -->
        <a href="javascript:;"><i data-command="multiSelect" class="command iconfont icon-select" title="多选"></i></a>
        <!-- <i data-command="addGroup" class="command iconfont icon-group disable" title="成组"></i>
        <i data-command="unGroup" class="command iconfont icon-ungroup disable" title="解组"></i> -->
            <a href="javascript:;"><span class="cut custom iconfont icon-cut-off"></span></a>
        <a href="javascript:;"><span class="custom iconfont icon-view-show" title="预览" @click="previewHandle"></span></a>
        <a href="javascript:;"><span class="custom iconfont icon-x" title="下载" @click="downloadHandle"></span></a>

      
      </div>
      <div class="data-opt">
        <Button type="success" @click="saveFlow">保存</Button>
      </div>
    </div>
    <div class="page-container">
      <div class="left-pannel">
        <div class="itempannel-container" ref="itempannel">
          <img
            data-nodeNameNow="begin"
            draggable="false"
            src="data:image/svg+xml;base64,"
            data-type="node"
            data-shape="flow-circle"
            data-size="72*72"
            data-color="#33CC66"
            data-label="开始节点"
            class="getItem"
          />
          <img
            data-nodeNameNow="normal"
            draggable="false"
            src="data:image/svg+xml;base64,"
            data-type="node"
            data-shape="flow-rect"
            data-size="80*48"
            data-color="#1890FF"
            data-label="常规节点"
            class="getItem"
          />
          <img
            data-nodeNameNow="branch"
            draggable="false"
            src="data:image/svg+xml;base64,"
            data-type="node"
            data-shape="flow-rhombus"
            data-size="80*72"
            data-color="#13C2C2"
            data-label="分叉节点"
            class="getItem"
          />
          <img
            data-nodeNameNow="model"
            draggable="false"
            src="data:image/svg+xml;base64,"
            data-type="node"
            data-shape="flow-capsule"
            data-size="80*48"
            data-color="#722ED1"
            data-label="模型节点"
            class="getItem"
          />
          <img
            data-nodeNameNow="over"
            draggable="false"
            src="data:image/svg+xml;base64,"
            data-type="node"
            data-shape="flow-circle"
            data-size="72*72"
            data-color="#999900"
            data-label="结束节点"
            class="getItem"
          />  
          <img
            data-nodeNameNow="mark"
            draggable="false"
            src="data:image/svg+xml;base64,"
            data-type="node"
            data-shape="flow-circle"
            data-size="36*36"
            data-color="rgb(153,0,0)"
            data-label="标识"
            class="getItem"
          />                  
        </div>
      </div>
      <div class="center-pannel">
        <div class="flow" ref="flow"></div>
        <div class="contextmenu" ref="contextmenu" style="position: absolute; z-index:2;">
          <div data-status="node-selected" class="menu" style="display: none;">
            <div data-command="copy" class="command disable">
              <span>复制</span>
              <!-- <span>copy</span> -->
            </div>
            <div data-command="delete" class="command disable">
              <span>删除</span>
            </div>
          </div>
          <div data-status="edge-selected" class="menu" style="display: none;">
            <div data-command="delete" class="command disable">
              <span>删除</span>
            </div>
          </div>
          <div data-status="group-selected" class="menu" style="display: none;">
            <div data-command="copy" class="command disable">
              <span>复制</span>
            </div>
            <div data-command="delete" class="command disable">
              <span>删除</span>
            </div>
            <div data-command="unGroup" class="command disable">
              <span>解组</span>
            </div>
          </div>
          <div
            data-status="canvas-selected"
            class="menu"
            style="display: block;"
          >
            <div data-command="undo" class="command disable">
              <span>撤销</span>
            </div>
            <div data-command="redo" class="command disable">
              <span>重做</span>
            </div>
            <div data-command="pasteHere" class="command disable">
              <span>粘贴</span>
            </div>
          </div>
          <div data-status="multi-selected" class="menu" style="display: none;">
            <div data-command="copy" class="command disable">
              <span>复制</span>
            </div>
            <div data-command="paste" class="command disable">
              <span>粘贴</span>
            </div>
            <!-- <div data-command="addGroup" class="command disable">
              <span>归组</span>
            </div> -->
            <div data-command="delete" class="command disable">
              <span>删除</span>
            </div>
          </div>
        </div>
      </div>
      <div class="right-pannel">
        <div class="detail-pannel">
          <div ref="detailpannel">
            <div
              data-status="node-selected"
              class="pannel"
              style="display: none;"
            >
              <div class="pannel-title">编辑-节点</div>
              <div class="block-container">
                <Row>
                  <Col class="label" span="6"><label>名称：</label></Col>
                  <Col span="18"><Input v-model="nodeName" :maxlength="30"/></Col>
                </Row>
                <Row>
                  <Col class="label" span="6"><label>尺寸：</label></Col>
                  <Col span="18"><Input type="number" :number="true" v-model="nodeWidth" :maxlength="5"/></Col>
                </Row>
                <Row>
                  <Col offset="6" span="18"><Input type="number" :number="true" v-model="nodeHeight" :maxlength="5"/></Col>
                </Row>  
                <Row>
                    <Col class="label" span="6"><label>背景色：</label></Col>
                    <Col span="18"><ColorPicker v-model="nodeColor" /></Col>
                </Row>
                <!-- <Row>
                    <Col class="label" span="6"><label>文本色：</label></Col>
                    <Col span="18"><ColorPicker v-model="nodeLabelColor" /></Col>
                </Row>                 -->
              </div>
            </div>
            <div data-status="edge-selected" class="pannel" id="edge_detailpannel" style="display: none;">
              <div class="pannel-title">编辑-边</div>
              <div class="block-container">
                <Row>
                    <Col class="label" span="6"><label>名称：</label></Col>
                    <Col span="18"><Input v-model="edgeName" :maxlength="30"/></Col>
                </Row>
                <Row>
                    <Col class="label" span="6"><label>线形：</label></Col>
                    <Col span="18">
                        <Select v-model="edgeShape">
                            <Option value="flow-smooth">曲线</Option>
                            <Option value="flow-polyline">折线</Option>
                            <Option value="flow-polyline-round">折线（圆角）</Option>
                        </Select>                    
                    </Col>
                </Row>
                <Row>
                    <Col class="label" span="6"><label>背景色：</label></Col>
                    <Col span="18"><ColorPicker v-model="edgeColor" /></Col>
                </Row>
              </div>
            </div>
            <div data-status="group-selected" class="pannel" id="group_detailpannel" style="display: none;">
              <div class="pannel-title">编辑-组</div>
              <div class="block-container">
                <Row>
                  <Col class="label" span="6"><label>名称：</label></Col>
                  <Col span="18"><Input v-model="groupName" /></Col>
                </Row>

                <!-- <div class="p color">
                  颜色：
                  <ColorPicker
                    v-model="groupColor"
                   
                  ></ColorPicker>
                </div> -->
              </div>
            </div>
            <div data-status="canvas-selected" class="pannel" id="canvas_detailpannel" style="display: none;">
              <div class="pannel-title">画布</div>
              <div class="block-container">
                <Checkbox v-model="gridCheck">网格对齐</Checkbox >
              </div>
            </div>
            <div data-status="multi-selected" class="pannel" id="multi_detailpannel" style="display: block;">
              <div class="pannel-title">多选</div>
              <div class="block-container">
                <!-- <div class="p color">
                  颜色：
                  <ColorPicker
                    v-model="multiColor"
                   
                  ></ColorPicker>
                </div> -->
                
              </div>
            </div>
          </div>
        </div>
        <div class="navigator">
          <span class="pannel-title">导航器</span>
          <div class="mini-map" ref="minimap"></div>
        </div>
        <div class="zoom-slider">
          <Slider
            v-model="zoomRatio"
            show-input
            input-size="small"
            :max="200"
            :format-tooltip="formatTooltip"
          ></Slider>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import G6Editor from '@antv/g6-editor'
// import FLowData from '@/model/flow'
// import JsPDF from 'jspdf'
import './assets/icon/font.css'


export default {
  name: 'FlowBase',
  props: {
    data: Object,       // 传入数据
    getData: Function   // 获取数据
  },
  components: {},
  data() {
    return {
      nowType:'',        // 当前类型
      nodeNameNow:'',    // 当前节点 名称

      page: null,
      flow: null,
      graph: null,

      // 节点 - 属性
      nodeId: null,
      nodeName: null,
      nodeWidth: null,
      nodeHeight: null,
      nodeColor: '#fff', 
      nodeLabelColor:'#fff',      

      groupId: null,
      groupName: null,
      groupColor: null,

      // 边-属性
      edgeId: null,
      edgeName: null,  
      edgeShape: '',  // 边-边形
      edgeColor:'#79838e',  

      multiId: [], // 多选模式选中的node节点id
      multiColor: null, // 多选模式下的color，仅以最后一个为代表颜色
      isMultiSelect: false, // 是否是多选模式
      gridCheck: true,
      zoomRatio: 100
    }
  },
  mounted() {
    this.initEditor()
  },
  watch: {
    multiColor(newValue) {
      this.multiId.forEach(id => {
        this.flow.update(id, {
          color: newValue
        })
      })
    },
    /**
     * 监听 节点属性 
     * */
    nodeName(newValue) {
      this.flow.update(this.nodeId, {
        label: newValue
      })
    },
    nodeWidth(newValue) {
      this.flow.update(this.nodeId, {
        size: `${newValue}*${this.nodeHeight}`
      })
    },
    nodeHeight(newValue) {
      this.flow.update(this.nodeId, {
        size: `${this.nodeWidth}*${newValue}`
      })
    },
    nodeColor(newValue, oldValue) {
      if (oldValue === null) return
      this.flow.update(this.nodeId, {
        color: newValue
      })
    },
    /**
     * 监听 边属性
     * */
    edgeName(newValue) {
      if (newValue === null) return
      this.flow.update(this.edgeId, {
        label: {
          text: newValue
        }
      })
    },
    edgeShape(newValue) {
      if (newValue === null) return
      this.flow.update(this.edgeId, {
        shape:newValue
      })
    },
    edgeColor(newValue, oldValue) {
      if (oldValue === null) return
      this.flow.update(this.edgeId, {
        style: {
          stroke: newValue
        }
      })
    },
    /** 
     * 分组
    */
    groupName(newValue) {
      if (newValue === null) return
      this.flow.update(this.groupId, {
        label: newValue
      })
    },
    groupColor(newValue) {
      if (newValue === null) return
      this.flow.update(this.groupId, {
        color: newValue
      })
    },
    gridCheck(newValue) {
      newValue ? this.page.showGrid() : this.page.hideGrid()
    },
    /**
     * 导航
     */
    zoomRatio(newValue) {
      this.graph.zoom(newValue / 100)
    }
  },
  methods: {
    /**
     * @description: 初始化编辑器
     */
    initEditor: function(){
      const editor = new G6Editor();
      let that = this;

      // 关闭追踪反馈
      G6Editor.track(false);

      // 基础流程图
      this.flow = new G6Editor.Flow({
        graph: {
          container: this.$refs.flow
        },
        align: {
          grid: true // 网格对齐
        },
        grid: {
          cell: 20 // 网孔尺寸
        },
        shortcut: {
          zoomIn: true, // 开启放大快捷键
          zoomOut: true // 开启视口缩小快捷键
        }
      });

      // 流程图图类
      this.itempannel = new G6Editor.Itempannel({
        container: this.$refs.itempannel
      });

      // 缩略图
      this.minimap = new G6Editor.Minimap({
        container: this.$refs.minimap,
        height:190
      });

      // 命令工具
      this.toolbar = new G6Editor.Toolbar({
        container: this.$refs.toolbar
      });

      // 右键菜单
      this.contextmenu = new G6Editor.Contextmenu({
        container: this.$refs.contextmenu
      });

      // 详细信息面板
      this.detailpannel = new G6Editor.Detailpannel({
        container: this.$refs.detailpannel
      });
      
      // 通过editor添加关联
      editor.add(this.flow);
      editor.add(this.itempannel);
      editor.add(this.minimap);
      editor.add(this.toolbar);
      editor.add(this.contextmenu);
      editor.add(this.detailpannel);

      // 增强清晰度
      // const canvas = this.$refs.flow.childNodes[0].childNodes[0]
      // const clarity = 1.5 // 清晰度
      // const dpr = window.devicePixelRatio && window.devicePixelRatio >= clarity
      //   ? window.devicePixelRatio : clarity
      // const rect = canvas.getBoundingClientRect()
      // canvas.width = rect.width * dpr
      // canvas.height = rect.height * dpr
      // const ctx = canvas.getContext('2d')
      // ctx.scale(dpr, dpr)

      // 判断是否是多选模式
      editor.on('aftercommandexecute', ev => {
        switch (ev.command.name) {
          case 'multiSelect':
            this.multiId = [] // 多选前清空上一个状态
            this.isMultiSelect = true
            break
          case 'undo':
            this.isMultiSelect = false
            break
          default:
            break
        }
      })

      // 流图读取数据
      this.data && this.flow.read(this.data);

      // 获取流图的 graph示例
      this.graph = this.flow.getGraph();

      // 居中画布中的内容
      this.graph.setFitView('cc');

      // 边形状映射
      this.graph.edge({
        // style: {
        //   stroke: that.edgeColor
        //   // stroke: '#79838e'
        // },
        shape:'',
        labelRectStyle: {
          fill: '#ffffff'
        }
      });

      // 获取当前页
      this.page = editor.getCurrentPage();

      // 关闭网格对齐
      // this.page.hideGrid();

      // 修改添加边的模型
      // this.page.changeAddEdgeModel({
      //     shape: that.edgeShape || 'flow-smooth',
      // });

      // 选中数据处理
      this.page.on('afteritemselected', ev => {

        that.nowType = ev.item.type;   // 当前类型


        // console.log( ev.item );
        // console.log( '999999999999' );

        // if( ev.item.model.nodenamenow == 'mark' ){
        //   this.flow.update(ev.item.model.id, {
        //     fill:'#32aa66',
        //     opacity:1,
        //     color: '#32aa66',
        //     label: {
        //       text: 123,
        //       fill:'#fff'
        //     }
        //   })
        // }




        // 判断数据类型
        switch (ev.item.type) {
          case 'node':
            if (!this.isMultiSelect) {
              this.nodeId = ev.item.model.id
              this.nodeName = ev.item.model.label;
              [this.nodeWidth, this.nodeHeight] = ev.item.model.size.split('*');
              this.nodeColor = ev.item.model.color;
            } else {
              this.multiId.push(ev.item.model.id);
              this.multiColor = ev.item.model.color;
            }

            // get 当前节点节点 
            that.nodeNameNow = ev.item.model.nodenamenow;

            break
          case 'edge':
            this.edgeId = ev.item.model.id;
            that.edgeColor = (ev.item.model.style ? ev.item.model.style.stroke :'#79838e');
            that.edgeShape = (ev.item.model.shape || 'flow-smooth');
            this.edgeName = ev.item.model.label ? ev.item.model.label.text : null
            break
          case 'group':
            this.groupId = ev.item.model.id
            this.groupName = ev.item.model.label ? ev.item.model.label : null
            this.groupColor = ev.item.model.color
              ? ev.item.model.color
              : '#f2f4f5'
            break
          default:
            break
        }

        // console.log(1111111111111111111111111111111);
        // console.log(that.nowType);
        // console.log(that.nodeNameNow);
      })

      // 取消多选
      this.page.on('beforeitemunselected', () => {
        this.isMultiSelect = false
      })

      // 自动更新
      this.graph.on('afterchangesize', () => {
        this.graph.update()
      })
    },
    /**
     * @description: 保存流图数据
     */
    saveFlow: function(){

      // graph.downloadImage();
      console.log(this);
      const data = this.flow.save();
      this.getData(data);
      // this.$message({
      //   message: '数据保存成功！',
      //   type: 'success'
      // })
    },
    /**
     * @description: 下载流图
     */
    previewHandle: function() {
      this.page.hideGrid();
      // this.graph.setFitView('cc');
      const canvas = this.$refs.flow.childNodes[0].childNodes[0];
      const imgData = canvas.toDataURL('image/png');


      const img = new Image();
      img.src = imgData;
      const newWin = window.open("", "_blank");
      newWin.document.write(img.outerHTML);
      newWin.document.title = '流程图-预览';
      newWin.document.close();


      this.page.showGrid();
      // const pdf = new JsPDF({
      //   orientation: 'l', // 竖排
      //   unit: 'pt',
      //   format: 'a4'
      // })
      // // const width = 1190.55 // a3
      // const pdfWidth = 840.51 // a4
      // pdf.addImage(imgData, 'PNG', 0, 0,
      // pdfWidth, pdfWidth / canvas.width * canvas.height, '', 'FAST');
      // pdf.save('download.pdf');
    },
    /**
     * @description: 下载
     */
    downloadHandle:function(){
      this.page.hideGrid();
      // this.graph.setFitView('cc');
      const canvas = this.$refs.flow.childNodes[0].childNodes[0];


      function download(url, name) {
          const aLink = document.createElement('a');
          aLink.download = name;
          aLink.href = url; 
          aLink.dispatchEvent(new MouseEvent('click', {}));
      }


      download(canvas.toDataURL('image/png'), '流程图.png');
      this.page.showGrid();

    },
    /**
     * @description: 格式化
     */
    formatTooltip: function(val){
      return `${val}%`
    }
  }
}
</script>

